...
@section('css')
    <style>
        ...
        .admin-filter-bar .form-select {
            border-radius: .5rem !important; border: 1px solid #e2e5f1;
        }
        ...
        .admin-filter-bar .btn-filter {
            border-radius: .5rem !important; padding: .375rem 1rem;
        }
        .admin-filter-tags {
            display: flex; align-items: center; gap: 8px; flex-wrap: wrap; padding: 12px 0 0;
        }
        .admin-filter-tag {
            display: inline-flex; align-items: center; gap: 6px;
            padding: 6px 14px; border-radius: 999px;
            background: rgba(15, 75, 191, 0.10); color: #0f4bbf;
        ...
        .admin-filter-count {
            margin-inline-start: auto; font-size: 0.8rem; color: #6b7280; white-space: nowrap;
        }
        .search-clear-btn {
            border: 1px solid #e2e5f1; border-left: 0;
            border-radius: 0 .5rem .5rem 0 !important;
        ...
        .admin-pagination .page-link:hover {
            background: #f3f6ff; border-color: #c4d0e3; color: #0f4bbf;
        }
        ...
        <div class="container-fluid">
            ...
            <div class="card admin-content-card">
                ...
                <div class="card-body" style="padding-bottom:0;">
                    <form method="GET" action="{{ route('admin/categories/index', ['offset' => 0, 'limit' => PAGINATION_COUNT]) }}" id="categoriesFilterForm">
                        <div class="admin-filter-bar">
                            <div class="input-group" style="min-width:200px;">
                                <span class="input-group-text bg-transparent"><i class="ri-search-line"></i></span>
                                <input type="text" name="search" value="{{ $search ?? '' }}" class="form-control" placeholder="{{ __('admin.pages.common.search_in', ['label' => __('admin.nav.categories')]) }}" id="categoriesSearchInput">
                                @if(($search ?? '') !== '')
                                <button type="button" class="search-clear-btn" onclick="clearCategoriesSearch()">&times;</button>
                                @endif
                            </div>
                            ...
                            @if($hasActiveFilters)
                                <a href="{{ route('admin/categories/index') }}/0/{{ PAGINATION_COUNT }}" class="btn btn-light btn-filter">
                                    <i class="ri-close-line align-bottom"></i>
                                </a>
                            @endif
                            <span class="admin-filter-count">
                                <i class="ri-grid-line align-bottom me-1"></i>
                                {{ $categories->total() }}
                            </span>
                        </div>
                        @if($hasActiveFilters)
                        <div class="admin-filter-tags">
                            ...
                            <span class="admin-filter-tag">
                                ...
                                {{ $search }}
                                <a href="{{ route('admin/categories/index') }}/0/{{ PAGINATION_COUNT }}" class="btn-close-sm">&times;</a>
                            </span>
                            @endif
                            <a href="{{ route('admin/categories/index') }}/0/{{ PAGINATION_COUNT }}" class="admin-filter-tag is-reset">
                ...
                <div class="card-body">
                    <div class="table-responsive">
                        <table id="scroll-horizontal" class="table nowrap align-middle dataTable no-footer" style="width: 100%" aria-describedby="scroll-horizontal_info">
                            ...
                            <tbody id="tableShowData">
                                ...
                                @empty
                                    <tr>
                                        <td colspan="5" class="text-center text-muted py-5">{{ __('admin.pages.common.no_data') }}</td>
                                    </tr>
                                @endforelse
                            </tbody>
                        </table>
                    ...
                    <div class="admin-pagination-wrap">
                        <div class="admin-pagination-info">
                            @php $from = $categories->firstItem(); $to = $categories->lastItem(); $total = $categories->total(); @endphp
                            {{ __('admin.pages.common.showing_entries', ['from' => $from, 'to' => $to, 'total' => $total]) }}
                        </div>
                        <nav aria-label="Page navigation">
                            <ul class="admin-pagination">
                                <li class="page-item {{ $categories->onFirstPage() ? 'disabled' : '' }}">
                                    <a class="page-link prev-next" href="{{ $categories->previousPageUrl() ?? '#' }}" aria-label="Previous">
                                        <i class="ri-arrow-left-s-line"></i> <span>{{ __('admin.pages.common.previous') }}</span>
                                    </a>
                                </li>
                                @php $current = $categories->currentPage(); $last = $categories->lastPage(); $start = max(1, $current - 2); $end = min($last, $current + 2); @endphp
                                @if($start > 1)
                                ...
                                <li class="page-item {{ !$categories->hasMorePages() ? 'disabled' : '' }}">
                                    <a class="page-link prev-next" href="{{ $categories->nextPageUrl() ?? '#' }}" aria-label="Next">
...
@section('script')
    <script>
        function clearCategoriesSearch() {
            document.getElementById('categoriesSearchInput').value = '';
            document.getElementById('categoriesFilterForm').submit();
        }
        @if(($search ?? '') !== '')
            document.addEventListener('DOMContentLoaded', function() {
                var input = document.getElementById('categoriesSearchInput');
