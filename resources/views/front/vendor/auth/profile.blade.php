    ...
    <main class="hp-profile-page">
        ...
        <div class="hp-profile-shell">
            <aside class="hp-profile-card hp-profile-sidebar">
                ...
                <a class="hp-profile-tab" data-target="#company-pane" href="#company-pane">
                    <i class="bi bi-building"></i>
                    <span>{{ __('profile.company_details') }}</span>
                </a>

                <a class="hp-profile-tab" data-target="#password-pane" href="#password-pane">
                    <i class="bi bi-shield-lock"></i>
                    <span>{{ __('profile.change_password') }}</span>
                </a>

                <a class="hp-profile-tab" data-target="#language-pane" href="#language-pane">
                ...
                <form method="POST" action="{{ route('vendor/logout') }}">
                    @csrf
                    <button class="hp-profile-logout" type="submit">
            ...
            <section class="hp-profile-card hp-profile-panel">
                <div class="hp-profile-pane is-active" id="profile-pane">
                    ...
                    <form action="{{ route('vendor/update') }}" method="post" enctype="multipart/form-data" class="hp-profile-form">
                        @csrf
                        ...
                        <div class="hp-profile-grid">
                            <div class="hp-profile-field hp-profile-field--full">
                                <label class="hp-profile-label">{{ __('profile.company_name') }}</label>
                ...
                <div class="hp-profile-pane" id="company-pane">
                    <div class="hp-profile-panel-head">
                        <div>
                            <h4>{{ __('profile.company_details') }}</h4>
                            <p>{{ __('profile.company_help') }}</p>
                        </div>
                        <span class="hp-profile-chip">
                            <i class="bi bi-buildings"></i>
                            {{ __('profile.company_info') }}
                        </span>
                    </div>
                    ...
                    <form action="{{ route('vendor/update') }}" method="post" enctype="multipart/form-data" class="hp-profile-form">
                        ...
                        <div class="hp-profile-grid">
                            <div class="hp-profile-field">
                                <label class="hp-profile-label">{{ __('profile.branch') }}</label>
                                <input name="branch" class="hp-profile-input" value="{{ old('branch', $user?->branch) }}">
                            </div>

                            <div class="hp-profile-field">
                                <label class="hp-profile-label">{{ __('profile.iban') }}</label>
                                <input name="iban" class="hp-profile-input" value="{{ old('iban', $user?->iban) }}">
                            </div>

                            <div class="hp-profile-field">
                                <label class="hp-profile-label">{{ __('profile.tax_number') }}</label>
                                <input name="tax_number" class="hp-profile-input" value="{{ old('tax_number', $user?->tax_number) }}">
                            </div>

                            <div class="hp-profile-field">
                                <label class="hp-profile-label">{{ __('profile.cr_number') }}</label>
                                <input name="cr_number" class="hp-profile-input" value="{{ old('cr_number', $user?->cr_number) }}">
                            </div>
                        </div>
                ...
                <div class="hp-profile-pane" id="password-pane">
                    ...
                    <form method="POST" action="{{ route('vendor/changePassword') }}" class="hp-profile-form">
                        @csrf
                        ...
                        <div class="hp-profile-field">
                            ...
                            <div class="hp-profile-password">
                                ...
                                <button class="hp-profile-eye toggle-pass" type="button">
                        ...
                        <div class="hp-profile-actions">
                            <button class="hp-btn-primary" type="submit">{{ __('profile.change_password') }}</button>
                        </div>
                    </form>
                </div>
                ...
                <div class="hp-profile-pane" id="language-pane">
                    ...
                    <form method="POST" action="{{ route('vendor/lang/update') }}" class="hp-profile-form">
                        @csrf
    ...
    <script>
        $(function () {
            ...
            const titles = {
                ...
                '#company-pane': @json(__('profile.company_details')),
